...
3. **설계 방향**
    - TempToken 신규 추가
    - 토큰 발급을 전담하는 경로 추가 (예. `/auth/token/issue`)
    - `authStatus`로 상태만 표현
...
### 토큰 발급 로직이 퍼질 위험이 있다  
 - 토큰을 실제로 만드는 건 `JwtService` 한 곳에서 하고 있다.
 - "토큰을 발급하는 시점"은 `LoginUsecase`(카카오 콜백)와 `RefreshTokenUsecase`(토큰 갱신) 두 군데로 나뉘어 있다
 - 발급을 트리거하는 코드가 각 도메인 UseCase로 흩어질 수 있다.
 - "발급 조건을 바꾸려면 어디를 고쳐야 하는가"가 더 이상 한 곳에서 결정되지 않는다.
...
정식 토큰이 발급되는 순간 이 임시 토큰은 소멸되고 재사용할 수 없다. DB에 저장되기 때문에 탈취·악용이 의심될 때 서버에서 강제로 막을 수도 있다.

### 토큰 발급처 통합

정식 `accessToken` / `refreshToken`은 오직 `POST /api/v1/auth/token/issue` 하나에서만 발급된다.

동의 처리나 전화번호 인증은 각자 할 일(저장 + 현재 상태 반환)만 하고 토큰 발급에는 관여하지 않는다. 덕분에 "발급 조건이 바뀌면 한 곳만 고치면 된다"는 구조가 만들어진다.
...
## 이점
 ...
 - 동의 UseCase는 동의만, 인증 UseCase는 인증만 책임진다. 토큰 발급은 한 곳에서만 일어난다.
...
전화번호 인증 (SMS 발송 → 코드 입력)
    │  authStatus: PHONE_VERIFICATION_REQUIRED
    ▼
...
약관 동의 × N
    │
    ▼
POST /auth/token/issue   ← 전화번호 인증 단계 스킵 (이미 인증된 회원)
    │
    ▼
정식 토큰 발급 (authStatus: COMPLETED)
```